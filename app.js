    ...
    Object.keys(groupedBySupplier).sort().forEach(supplier => {
            ...
            <div class="table-container">
                <table>
                    <thead>
                        <tr>
                            <th>Nome</th>
                            <th class="hide-mobile">Codice</th>
                            <th>Qty</th>
                            <th class="hide-mobile">Soglia</th>
                            <th class="hide-mobile">Pr. Acq. NETTO</th>
        ...
        supplierArticles.forEach(article => {
            ...
            row.innerHTML = `
                ...
                <td class="hide-mobile">${article.codice_articolo}</td>
                <td><strong>${article.quantita}</strong></td>
                <td class="hide-mobile">${article.soglia_minima}</td>
                <td class="hide-mobile">â‚¬ ${parseFloat(article.prezzo_acquisto).toFixed(2)}</td>
...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    // Mostra il banner solo se su mobile
...
// Gestisci il click sul pulsante "Installa App"
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (dismissBtn) {
        dismissBtn.addEventListener('click', () => {
            ...
            // Salva la preferenza per non mostrarlo di nuovo per un po'
            localStorage.setItem('pwa-banner-dismissed', Date.now().toString());
        });
    }
    
    // Controlla se il banner è stato già chiuso di recente
    const dismissed = localStorage.getItem('pwa-banner-dismissed');
    if (dismissed) {
        const dismissedTime = parseInt(dismissed);
        const now = Date.now();
        const sevenDays = 7 * 24 * 60 * 60 * 1000; // 7 giorni in millisecondi
        
        // Se sono passati meno di 7 giorni, nascondi il banner
        if (now - dismissedTime < sevenDays) {
            if (pwaBanner) {
                pwaBanner.classList.add('hidden');
            }
        } else {
            // Se sono passati più di 7 giorni, rimuovi la preferenza
